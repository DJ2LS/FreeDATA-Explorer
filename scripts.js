...
$.getJSON({
	...
	for (var i = 0; i < data.length; i++) {
		...
		try{
			...
			console.log(e)
			var latlon =  [0.00,0.00]
		}
		//Recorded in DB at UTC+1
		...
		if (lastHeard !== "" && lastHeard !== "null"   ){
			try{
				...
				for (const x in lastHeard) {
					...
					if (lastHeard[x]["callsign"].split('-', 1)[0] == callsign.split('-', 1)[0] && (lastHeard[x]["grid"].toLowerCase()== gridsquare.toLowerCase() || lastHeard[x]['grid'] == "------")) continue;
				try{
					var latlon_dx = gridSquareToLatLon(lastHeard[x]["grid"]);
					var dist_KM = Math.round(distance(latlon[0], latlon[1], latlon_dx[0], latlon_dx[1], 'K'));
					var dist_NM = Math.round(distance(latlon[0], latlon[1], latlon_dx[0], latlon_dx[1], 'N'));
					...
					//Recorded in DB as UTC
					var timestampLastHeard = new Date(lastHeard[x]["timestamp"]).getTime();
					var formattedTime = new Date(timestampLastHeard * 1000).toLocaleTimeString(locale);
					...
					if(x < 10){
						...
						} else if (lastHeard[x]["snr"] >= 10 && lastHeard[x]["snr"] <= 15){
							var LineColor = 'green';
			
						} else if (lastHeard[x]["snr"] >= 5 && lastHeard[x]["snr"] <= 10){
							var LineColor = 'green';
								
						} else if (lastHeard[x]["snr"] >= 2 && lastHeard[x]["snr"] <= 5){
							var LineColor = 'yellow';
														
						} else if (lastHeard[x]["snr"] >= 0 && lastHeard[x]["snr"] <= 2){
							var LineColor = 'orange';
						
						} else if (lastHeard[x]["snr"] >= -5 && lastHeard[x]["snr"] <= 0){
							var LineColor = 'red';
						...
						if(dist_KM > 0 && callsign_list.includes(lastHeard[x]["callsign"])){
							var polyline = L.polyline(latlngs, {color: LineColor, weight: 2,opacity: 0.5,smoothFactor: 1,}).addTo(Lines);
						}
		...
		if(frequency >= 28000000 && frequency <= 28500000){
			...
			L.marker([latlon[0], latlon[1]],{icon: iconColor}).addTo(Marker12m).bindPopup(popup, {
				maxWidth : 560
			});
		} else if (frequency >= 21000000 && frequency <= 21500000){
			L.marker([latlon[0], latlon[1]],{icon: iconColor}).addTo(Marker15m).bindPopup(popup, {
				maxWidth : 560
			});
		} else if (frequency >= 18000000 && frequency <= 18500000){
			L.marker([latlon[0], latlon[1]],{icon: iconColor}).addTo(Marker17m).bindPopup(popup, {
				maxWidth : 560
			});
		} else if (frequency >= 14000000 && frequency <= 14500000){
			L.marker([latlon[0], latlon[1]],{icon: iconColor}).addTo(Marker20m).bindPopup(popup, {
				maxWidth : 560
			});
		} else if (frequency >= 10000000 && frequency <= 10500000){
			L.marker([latlon[0], latlon[1]],{icon: iconColor}).addTo(Marker30m).bindPopup(popup, {
				maxWidth : 560
			});
		} else if (frequency >= 7000000 && frequency <= 7200000){
			L.marker([latlon[0], latlon[1]],{icon: iconColor}).addTo(Marker40m).bindPopup(popup, {
				maxWidth : 560
			});
		} else if (frequency >= 5300000 && frequency <= 5360000){
			L.marker([latlon[0], latlon[1]],{icon: iconColor}).addTo(Marker60m).bindPopup(popup, {
				maxWidth : 560
			});
		} else if (frequency >= 3500000 && frequency <= 3800000){
			L.marker([latlon[0], latlon[1]],{icon: iconColor}).addTo(Marker80m).bindPopup(popup, {
				maxWidth : 560
			});
		} else if (frequency >= 1800000 && frequency <= 2000000){
			L.marker([latlon[0], latlon[1]],{icon: iconColor}).addTo(Marker160m).bindPopup(popup, {
				maxWidth : 560
			});
		} else {
			L.marker([latlon[0], latlon[1]],{icon: iconColor}).addTo(MarkerOthers).bindPopup(popup, {
				maxWidth : 560
			});
		}
